  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex justify-between items-center h-16 ${isScrolled ? 'rounded-b-2xl' : ''}`}>
          <div className="flex items-center">
            <Leaf className={`h-8 w-8 text-primary`} />
            <span className={`ml-2 text-xl font-cinzel ${isInHeroSection ? 'text-white' : 'text-gray-800'}`}>
              Sustainable Futures
            </span>
          </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <a
                  ...
                  className={`${getTextColorClass(isActive)} transition-colors duration-200 relative group`}
                >
                  {item.label}
                  <motion.span 
          ...
          <div className="md:hidden">
            <button
              ...
              className={`${isInHeroSection ? 'text-white' : 'text-gray-800'} p-2 rounded-full hover:bg-white/10 transition-colors`}
            >
              <Menu size={24} />
            </button>
          </div>
        ...
        {isMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="md:hidden bg-black/80 backdrop-blur-md rounded-b-2xl overflow-hidden"
          >
              ...
              {navItems.map((item) => {
                ...
                return (
                  <a
                    ...
                    href={`#${item.id}`}
                    onClick={(e) => handleNavClick(e, item.id)}
                    className={`block py-2 px-4 ${isActive ? 'text-primary bg-white/10' : 'text-white'} hover:bg-primary/20 rounded-lg transition-colors`}
                  >
                    {item.label}
                  </a>
                );
